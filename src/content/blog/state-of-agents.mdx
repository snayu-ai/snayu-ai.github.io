...
# Optional: Connect it to Snayu cloud for effortless observability
import snayu

# Initialize a manager object, using snayu as observability provider
...
So we added built-in SFT and RL capabilities using GRPO to [chetan-serve](https://github.com/snayu-ai/chetan-serve), the official agent system server for chetan. With chetan-serve, we can host both, the language models and agent systems in one server.

Tighter integration between LMs and agent systems produces reduce latency, increase efficiency and yield better results.


<Tabs items={['chetan-serve', 'OpenAI']}>
  <Tab value="chetan-serve">
```python
from chetan.lm.serve import ChetanServeLM
